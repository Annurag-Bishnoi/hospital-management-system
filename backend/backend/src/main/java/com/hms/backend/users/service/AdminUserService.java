...
public class AdminUserService {
    ...
    public UserResponse createUser(CreateUserRequest request) {
        ...
        boolean passwordAutoGenerated = (rawPassword == null || rawPassword.isBlank());
        if (passwordAutoGenerated) {
            rawPassword = String.format("Staff@%06d", new java.util.Random().nextInt(999999));
        }
        ...
        UserResponse response = mapToUserResponse(savedUser, role);
        if (passwordAutoGenerated) {
    ...
    @Transactional
    public UserResponse autoResetCredentials(Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("User not found"));

        String newPassword = String.format("Staff@%06d", new java.util.Random().nextInt(999999));
        user.setPasswordHash(passwordEncoder.encode(newPassword));

        User savedUser = userRepository.save(user);
        UserResponse response = mapToUserResponse(savedUser);
        response.setTemporaryPassword(newPassword);
        return response;
    }
